...
func NewConverterWithOpts(opts *ConverterOpts) (*Converter, error) {
	...
	if settings == nil {
		return nil, ErrCreateGlobalSettings
	}

	// Create converter.
	cConverter := C.wkhtmltopdf_create_converter(settings)
	if cConverter == nil {
		return nil, ErrCreateConverter
	}
	...
	// Add converter to object syncMap.
	sm.Store(objectID(cConverter), converter)
...
func (c *Converter) Run(w io.Writer) error {
	if c.converter == nil {
		return ErrUninitializedConverter
	}
	if w == nil {
		return ErrUninitializedWriter
	}
...
func (c *Converter) Destroy() {
	...
	// Destroy converter.
	if c.converter != nil {
		sm.Delete(objectID(c.converter))
