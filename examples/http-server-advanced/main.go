...
type requestData struct {
	ConverterOpts *htmltopdf.ConverterOpts `json:"converterOpts"`
	ObjectOpts    *htmltopdf.ObjectOpts    `json:"objectOpts"`
}
...
func startServer() {
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		data := &requestData{
			ConverterOpts: htmltopdf.NewConverterOpts(),
			ObjectOpts:    htmltopdf.NewObjectOpts(),
		}
		...
		if err := callFunc(func() error {
			...
			object, err := htmltopdf.NewObjectWithOpts(data.ObjectOpts)
			if err != nil {
				return err
			}

			// Create converter with options.
			converter, err := htmltopdf.NewConverterWithOpts(data.ConverterOpts)
			if err != nil {
				log.Fatal(err)
			}
